...
router.post("/", passport.authenticate("jwt", { session: false }), (req, res) => {
  ...
  Profile.findOne({ user: req.user.id }).then(profile => {
    if (!profile) {
      res.status(404).json({ noprofile: "Please create a profile first to be able to post!" });
    } else {
...
router.post("/like/:post_id", passport.authenticate("jwt", { session: false }), (req, res) => {
  Profile.findOne({ user: req.user.id }).then(profile => {
    if (!profile) {
      res.status(404).json({ noprofile: "Please create a profile first to be able to like a post!" });
    } else {
      Post.findById(req.params.post_id)
        .then(post => {
...
router.post("/unlike/:id", passport.authenticate("jwt", { session: false }), (req, res) => {
  Profile.findOne({ user: req.user.id }).then(profile => {
    Post.findById(req.params.id)
      .then(post => {
        if (post.likes.filter(like => like.user.toString() === req.user.id).length === 0) {
          return res.status(400).json({ notyetlike: "You have not yet like this post." });
        }
...
router.post("/comment/:post_id", passport.authenticate("jwt", { session: false }), (req, res) => {
  ...
  Profile.findOne({ user: req.user.id }).then(profile => {
    if (!profile) {
      res.status(404).json({
        noprofile: "Please create a profile first to be able to unlike a post!"
      });
    } else {
      Post.findById(req.params.post_id)
        .then(post => {
